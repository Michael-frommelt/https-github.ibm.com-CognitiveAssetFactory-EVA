 ...
 * @param {*} cloudant The cloudant database that contains the answer store
 * @param {*} answerstore The name of the cloudant database that stores the answer texts.
 ...
 * @param {*} callback
 */
exports.run = function (cloudant, answerstore, assistant, workspace, target_parameter, id, callback) {
  ...
  assistant.getWorkspace(params, function (err, response) {
    ...
    // Get a list of all answers in the answerstore (we call cloudant only once, because the query rate is limited due to the used lite plan)
    cloudant.db.use(answerstore).find({ selector: {} }, function (err, result) {
      if (err) {
        return callback('Error while fetching data from database' + err);
      }
      if (result) {
        var answers = result["docs"];
        console.log("Got data from: " + answerstore);

        // Iterate over all nodes in the dialog tree
